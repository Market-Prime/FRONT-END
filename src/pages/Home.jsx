...
const Home = () => {
  const topdeal = Topdealdata.map((item, i) => {
    return (
      <div
        className="mx-auto  mb-8 2xl:w-1/6 xl:w-1/6 lg:w-1/6 md:w-1/2 sm:w-full"
        key={i}
      >
  ...
  const explore = ExploreData.map((items, i) => {
    return (
      <div
        key={i}
        className="mb-5 sm:w-full md:w-[32%] lg:w-72 xl:w-72 2xl:w-72"
      >
  ...
  const bestseller = Best.map((item, i) => {
    return (
      <div
        className="mx-auto bestbox mb-8 2xl:w-[15.9%] xl:w-[15.9%] lg:w-72 md:w-[45%] sm:w-full"
        key={i}
      >
        <img src={item.image} className="w-full mx-auto" />
